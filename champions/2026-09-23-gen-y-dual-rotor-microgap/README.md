# Gen-Y — Dual-rotor MICRO-GAP copper-window AFPM / wound-aware 12-coil 3φ

**PRIMARY challenger vs Gen-V** (night 2026-09-23 ET). Dual magnet faces across a **micro-gap** wound mid-stator (`m2m=5.76 mm` vs Gen-V `6.16`) + larger copper window (ID 21–OD 60) + 150 t restore B×N while magnet↔wound stays **0.65 mm PASS**.
...
| Turns | **150** / pancake | 12×150×~0.12 m ≈ **216 m** copper (fits 1 lb) |
...
| `former_web` / `flange_t` | **1.8 / 0.70 mm** | thinner than Gen-V 2.0/0.75 |
| `former_bare_h` | **3.2 mm** | |
| `coil_envelope_h` | **4.46 mm** | bare + 2×wind_build_axial |
| `run_clear` | **0.65 mm** | magnet → **wound** (each gap) |
| `gap_spacer_h` | **1.28 mm** | `run_clear + wind_build_axial` |
| `m2m` | **5.76 mm** | `coil_envelope_h + 2×run_clear` (**NOT** bare) |
| magnet→wound | **0.65 mm** | **PASS** both sides |
...
Brace with **M4 rods @ R≈68** **BEFORE** seating the second magnet face. Glue one rotor completely, dry-fit wound stator + gap_spacers, brace, then approach second rotor slowly. **Wind all 12 formers before assembly.** Micro-gap (0.65 mm) leaves less forgiveness — do not skip braces.

## Coils

- **12** pancakes on single mid stator (3φ × 4) — dual-gap each turn
- **26 AWG × 150 t**; star 3φ
- Mean turn ~0.12 m → copper budget ~216 m on 1 lb 26 AWG
...
rotor A magnet face
  └── run_clear 0.65 mm
wound copper (+Z face)
  └── coil_envelope_h 4.46 through mid stator
wound copper (−Z face)
  └── run_clear 0.65 mm
rotor B magnet face
—— m2m = 5.76 mm ——
...
Thinner m2m (+ larger window + +10 turns) vs Gen-V → B_gap Halbach-boosted ≈ **0.62–0.88 T** per gap. Each coil sees **two** magnet faces.
...
| **200** | **~14–28 V** | **~10–21 W** |
| 300 | ~21–42 V | ~18–35 W |
| 500–1000 | ~35–90 V | ~28–75 W (thermal) |

**vs Gen-V (~9–19 W @200):** Gen-Y targets **~10–21 W @200** — thinner envelope (5.76 vs 6.16) + more copper (150 vs 140 t, larger window ID21–OD60). Conservative range; validate with fit coupon + tach bench.
...
**0.4 mm** nozzle default. PETG/ABS preferred. Walls ≥0.8 (load 1.2). **DUAL-ROTOR CLAP.** Wind all formers first. Micro-gap demands brace rods.
