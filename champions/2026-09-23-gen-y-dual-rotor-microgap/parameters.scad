// =============================================================================
// Gen-Y Dual-rotor MICRO-GAP copper-window AFPM — WOUND-AWARE (HARD GATE)
// PRIMARY challenger vs Gen-V. Thinner m2m + larger copper window + 150 t.
// =============================================================================

// --- Wire / wind budget (26 AWG, ~1 lb spool ≈ 390 m bare-equiv) --------------
wire_awg            = 26;
wire_od             = 0.45;     // insulated OD mm
turns               = 150;      // +10 vs Gen-V
fill                = 0.70;
...
// --- Pancake former (bare STL) — thinner web/flange than Gen-V ---------------
coil_id_r           = 21.0;     // larger copper window vs Gen-V 23–58
coil_od_r           = 60.0;
...
former_bare_h       = former_web + 2 * flange_t;            // 3.2 mm
coil_envelope_h     = former_bare_h + 2 * wind_build_axial; // 4.46 mm

// --- Running clearance (magnet face → WOUND face) EACH side ------------------
run_clear           = 0.65;     // PASS ≥ 0.5 (prefer ≥0.65)
gap_spacer_h        = run_clear + wind_build_axial;         // 1.28 mm
m2m                 = coil_envelope_h + 2 * run_clear;      // 5.76 mm (beats Gen-V 6.16)
