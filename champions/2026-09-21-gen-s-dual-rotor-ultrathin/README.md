# Gen-S — Dual-rotor ULTRA-THIN AFPM / wound-aware 12-coil 3φ

**PRIMARY challenger vs Gen-P** (night 2026-09-21 ET). Dual magnet faces across an **ultra-thin** wound mid-stator (`m2m=6.66 mm` vs Gen-P `7.26`) + larger copper window + 130 t restore B×N while magnet↔wound stays **0.80 mm PASS**.
...
| Turns | **130** / pancake | 12×130×~0.11 m ≈ **172 m** copper (fits 1 lb) |
| fill | **0.70** | packing factor |
| layers_per_face | **2** | axial layers proud of each flange |
| `wind_build_axial` | **0.63 mm** | `2 × 0.45 × 0.7` |
| `wind_build_radial` | **0.945 mm** | `3 × 0.45 × 0.7` (extra turns radial) |
| `former_web` / `flange_t` | **2.2 / 0.8 mm** | thinner than Gen-P 2.6/0.9 |
| `former_bare_h` | **3.8 mm** | |
| `coil_envelope_h` | **5.06 mm** | bare + 2×wind_build_axial |
| `run_clear` | **0.80 mm** | magnet → **wound** (each gap) |
| `gap_spacer_h` | **1.43 mm** | `run_clear + wind_build_axial` |
| `m2m` | **6.66 mm** | `coil_envelope_h + 2×run_clear` (**NOT** bare) |
...
| Ø5×5 | **24** | Rotor A Halbach @ R={38,50,62} |
| Ø5×5 | **24** | Rotor B Halbach (+22.5°) |

Pockets: **Ø20.3×5.2**, **Ø5.3×5.2**. Carrier OD **148 mm**, thick **7.5 mm**.
...
Brace with **M4 rods @ R≈68** **BEFORE** seating the second magnet face. Glue one rotor completely, dry-fit wound stator + gap_spacers, brace, then approach second rotor slowly. **Wind all 12 formers before assembly.**

## Coils

- **12** pancakes on single mid stator (3φ × 4) — dual-gap each turn
- **26 AWG × 130 t**; star 3φ
- Mean turn ~0.11 m → copper budget ~172 m on 1 lb 26 AWG
...
rotor A magnet face
  └── run_clear 0.80 mm
wound copper (+Z face)
  └── coil_envelope_h 5.06 through mid stator
wound copper (−Z face)
  └── run_clear 0.80 mm
rotor B magnet face
—— m2m = 6.66 mm ——
...
Thinner m2m (+ larger window + +10 turns) vs Gen-P → B_gap Halbach-boosted ≈ **0.58–0.80 T** per gap. Each coil sees **two** magnet faces.
...
| **200** | **~11–22 V** | **~8–17 W** |
| 300 | ~17–33 V | ~14–28 W |
| 500–1000 | ~28–75 V | ~22–60 W (thermal) |

**vs Gen-P (~7–15 W @200):** Gen-S targets **~8–17 W @200** — thinner envelope (6.66 vs 7.26) + more copper area/turns. Conservative range; validate with fit coupon + tach bench.
...
**0.4 mm** nozzle default. PETG/ABS preferred. Walls ≥0.8 (load 1.2). **DUAL-ROTOR CLAP.** Wind all formers first.
