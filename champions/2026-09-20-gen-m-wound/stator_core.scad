// Gen-M stator core — print ×2
// Coil bays sized for WOUND envelope (radial + axial), not bare former.
...
module coil_bay(i) {
    ...
    rotate([0, 0, a])
        translate([0, 0, stator_thick - bay_depth])
            linear_extrude(bay_depth + 0.2)
                offset(r=bay_offset)
                    polygon([
