// Gen-X vernier rotor B — pockets +22.5°; print ×1
// DUAL-ROTOR CLAP: brace BEFORE approaching this face
include <parameters.scad>;
$fn = 72;

module large_pockets() {
    for (i = [0 : large_n-1]) {
        a = i * 360 / large_n + rotor_b_offset_deg;
...
module claw_smalls() {
    for (i = [0 : large_n-1]) {
        a = i * 360 / large_n + rotor_b_offset_deg;
        for (r = [R_claw_inner, R_claw_outer])
            translate([r*cos(a), r*sin(a), carrier_thick - pocket_h])
                cylinder(d=small_pocket_d, h=pocket_h + 0.15);
        a2 = a + 180/large_n;
        translate([R_claw_mid*cos(a2), R_claw_mid*sin(a2), carrier_thick - pocket_h])
            cylinder(d=small_pocket_d, h=pocket_h + 0.15);
    }
}
difference() {
