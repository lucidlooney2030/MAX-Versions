// =============================================================================
// Gen-AB Dual-rotor ULTRA-MICRO-GAP copper-max AFPM — WOUND-AWARE (HARD GATE)
// PRIMARY challenger vs Gen-Y. Thinner m2m + larger copper window + 160 t.
// =============================================================================

// --- Wire / wind budget (26 AWG, ~1 lb spool ≈ 390 m bare-equiv) --------------
wire_awg            = 26;
wire_od             = 0.45;     // insulated OD mm
turns               = 160;      // +10 vs Gen-Y 150
fill                = 0.70;
...
// --- Pancake former (bare STL) — thinner web/flange than Gen-Y ---------------
coil_id_r           = 20.0;     // larger copper window vs Gen-Y 21–60
coil_od_r           = 62.0;
span_deg            = 26.0;     // 12 stations × 30° pitch
former_web          = 1.6;      // print-safe min
...
former_bare_h       = former_web + 2 * flange_t;            // 2.8 mm
coil_envelope_h     = former_bare_h + 2 * wind_build_axial; // 4.06 mm

// --- Running clearance (magnet face → WOUND face) EACH side ------------------
run_clear           = 0.58;     // PASS ≥ 0.5 (prefer ≥0.65; ultra-micro 0.55–0.60 band)
gap_spacer_h        = run_clear + wind_build_axial;         // 1.21 mm
m2m                 = coil_envelope_h + 2 * run_clear;      // 5.22 mm (beats Gen-Y 5.76)
